...
import { useAOS, tryAdminLogin, adminLogout, adminAddPhotocard, adminRemovePhotocard, adminBanUser, adminSetCardRarity, todayISO, Rarity } from "@/lib/aos";
    ...
    <div className="fixed inset-0 z-50 bg-background overflow-y-auto">
      ...
      <div className="p-4 space-y-4 pb-24">
        ...
        <div className="rounded-2xl bg-white p-4 shadow">
          ...
          <div className="text-xs text-muted-foreground mb-2">Tap a card to assign its rarity. Changes save instantly for everyone on this device.</div>
          <div className="grid grid-cols-2 gap-3">
            {s.admin.library.map((c) => (
              <div key={c.id} className="relative rounded-xl bg-muted/40 p-2">
                ...
                <button onClick={() => { if (confirm("Remove?")) adminRemovePhotocard(c.id); }} className="absolute top-3 right-3 bg-destructive text-destructive-foreground rounded-full w-6 h-6 text-xs">✕</button>
                <div className="mt-2">
                  <div className="text-[10px] uppercase tracking-wide text-muted-foreground mb-1">Assign rarity</div>
                  <div className="flex flex-wrap gap-1">
                    {(["common", "rare", "ultra", "impossible"] as Rarity[]).map((r) => (
                      <button
                        key={r}
                        onClick={() => adminSetCardRarity(c.id, r)}
                        className={`px-2 py-1 rounded-full text-[11px] font-medium ${c.rarity === r ? "bg-primary text-primary-foreground" : "bg-white border"}`}
                      >
                        {r === "ultra" ? "ultra rare" : r}
                      </button>
                    ))}
                  </div>
                </div>
