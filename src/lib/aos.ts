...
function applyOverrides(lib: Photocard[], overrides?: Record<string, Rarity>): Photocard[] {
  if (!overrides) return lib;
  return lib.map((c) => (overrides[c.id] ? { ...c, rarity: overrides[c.id] } : c));
}
...
function load(): AOSData {
  ...
  try {
    ...
    const merged = mergeLibrary(base, BUNDLED_SHARED);
    parsed.admin = {
      ...parsed.admin,
      library: applyOverrides(merged, parsed.admin?.rarityOverrides),
    };
    return parsed;
  } catch {
    return defaultState();
...
export async function loadSharedLibrary() {
  ...
  try {
    ...
    const merged = mergeLibrary(state.admin.library, remote);
    state = {
      ...state,
      admin: {
        ...state.admin,
        library: applyOverrides(merged, state.admin.rarityOverrides),
      },
    };
    save();
  } catch {
...
export function adminSetCardRarity(cardId: string, rarity: Rarity) {
  const overrides = { ...(state.admin.rarityOverrides || {}), [cardId]: rarity };
  const library = state.admin.library.map((c) => (c.id === cardId ? { ...c, rarity } : c));
  state = { ...state, admin: { ...state.admin, library, rarityOverrides: overrides } };
  save();
}
